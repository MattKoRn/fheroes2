...
Offline progress can also **mobilize creatures from owned settlements indefinitely**. There is no elapsed-time ceiling: each built dwelling accrues recruits at a deliberately slow rate equal to one normal week of base growth per 35 real-world days, adjusted by saved kingdom-state efficiency. Recruitment starts after 12 hours for tier 1, then unlocks tiers 2–5 at 18, 24, 36, and 72 hours; tier 6 requires a full 7-day absence. Normal creature prices are paid, no more than 25% of the current treasury is used per offline return, and army-slot limits still apply. Offline gold income is boosted to 175% of the kingdom's normal saved gold income so long-running recruitment can fund itself without similarly inflating rare resources.

The state file is upgraded to version 8 to store the persistent creature roster and overflow reserve alongside the Supply Rush meter, map/player-state snapshot, treasure fragments, contracts, Renown, streaks, and lifetime offline seconds. Version-1 through version-7 files migrate automatically; their creature roster begins empty until the next snapshot. Exact resources, uncapped elapsed time, and fractional carry remain compatible.


## Persistent creatures and adaptive enemy scaling

Every surviving creature in the persistent player's hero armies and castle garrisons is snapshotted into the offline state. On a genuinely new map, that roster replaces the map's starting player troops. If the new map does not provide enough army slots for every distinct carried creature type, the remainder stays in a persistent reserve instead of being deleted; later snapshots combine deployed creatures and reserve creatures again. Loading an ordinary save does not re-import the roster, preventing duplication.

New-map enemy scaling compares the carried roster's actual monster strength with the map's normal starting player army strength. The multiplier follows a softened square-root curve and is capped at 2.5x for enemy hero/castle armies; neutral wandering monster stacks receive 70% of that extra scaling and are capped at 2.0x. Small carry-over armies therefore cause little or no increase, while extremely powerful persistent armies face progressively stronger opposition.
