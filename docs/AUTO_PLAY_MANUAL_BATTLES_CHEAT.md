...
The state file is upgraded to version 4 to store the active contract, its progress/target, and the completed-contract count in addition to Renown, streaks, and lifetime offline seconds. Version-1, version-2, and version-3 files migrate automatically; exact resources, uncapped elapsed time, base offline-income calculation, and fractional carry remain compatible.
