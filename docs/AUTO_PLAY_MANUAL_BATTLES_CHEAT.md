...
Offline progress can also **mobilize creatures from owned settlements indefinitely**. There is no elapsed-time ceiling: every built creature tier is eligible immediately, and each dwelling accrues recruits at a deliberately slow rate equal to one normal week of base growth per 35 real-world days, adjusted by saved kingdom-state efficiency. Fractional creature progress is persisted between offline sessions, so short absences still contribute without granting premature high-tier creatures. Normal creature prices are paid, but no treasury percentage is reserved: recruitment can spend down to the actual affordability limit. If resources or army slots prevent deployment, earned whole-creature progress remains banked for a later return. Offline gold income remains boosted to 175% of normal saved gold income.
