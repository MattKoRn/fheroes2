...
Offline progress can also **mobilize creatures from owned settlements**. This is deliberately not extra dwelling growth: recruitment only uses creatures already available in built dwellings and pays their normal resource cost. Recruitment starts after 12 hours for low tiers and unlocks progressively through 72 hours for the highest tier. Per dwelling, a return is capped at roughly half one normal week's base growth, scaled by the saved kingdom-state efficiency; the system also reserves at least 80% of the current treasury by limiting offline recruitment to a 20% mobilization budget. It prefers stronger creatures first, respects garrison/hero army space, and never recruits from more settlements than were present in the saved offline state.
