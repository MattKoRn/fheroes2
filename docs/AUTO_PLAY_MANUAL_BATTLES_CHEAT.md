...
Writes are transactional. If the primary snapshot is unusable after an interrupted save, recovery checks the fully written temporary snapshot before the older backup so the newest valid state is preferred.
...
Productive returns of at least 30 minutes also build a persistent **Supply Rush** meter from 0 to 100. Every return grants meter progress from time away, homecoming tier, and saved map/player-state efficiency, so stronger kingdoms charge the meter faster. When it reaches 100, Supply Rush triggers automatically, consumes 100 meter points, and grants +20% of that session's base offline production across every resource that produced income. The meter carries its remainder forward, the payout respects resource caps, and triggering a Rush also grants bonus Renown. The first offline popup remains compact and shows time away and resource rewards; it is followed by a separate **Offline Recruitment** popup summarizing creatures recruited during the return. The input state from the first modal is cleared before opening the second so the dismissal click/key cannot immediately close the recruitment summary. The recruitment grid shows full, exact creature counts rather than abbreviated values. Minimizing/backgrounding the application snapshots the persistent kingdom immediately, pauses gameplay and game/UI timers while unfocused, discards input accumulated in the background, and applies the exact background interval on resume. Resume rewards are applied on top of the live wallet instead of restoring an older wallet snapshot, so battle purchases and AI actions completed after returning are not rolled back.

Offline creature recruitment now works as an **unlimited-stock auto-buy pass on every offline return**. A built dwelling unlocks its creature type, but its current population is ignored. Eligible creatures from every owned town are ranked by **army-strength return on resource investment**, using marketplace-equivalent resource values; tier is only a tie-breaker. The system then buys the maximum count the current treasury can afford at normal creature prices. Recruit dialogs calculate affordability directly instead of incrementing one creature at a time, avoiding very long stalls on resource-rich saves. Purchased creatures are deployed immediately across owned castle and hero armies using overflow-safe 32-bit stack handling. Existing matching stacks are filled only to the uint32 limit, then additional free slots and other owned armies are used. Only creatures that genuinely cannot fit anywhere remain in the persistent reserve. The second offline popup records only purchases that were actually deployed or persisted, displays a compact grid of creature sprites with recruited counts, supports right-click creature details, and scales the grid to the current display height so it stays on-screen. No treasury percentage is reserved. The old version-9 fractional creature-recruitment bank is cleared and no longer affects purchases. Offline gold income remains boosted to 750% of normal saved gold income.
