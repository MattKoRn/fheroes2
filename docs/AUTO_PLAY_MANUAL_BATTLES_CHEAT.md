...
Offline creature recruitment now works as an **auto-buy pass on every offline return**. Instead of generating creatures from real-world time, the system scans each owned settlement's actual built dwellings and buys as many creatures as the current treasury can afford from the population already waiting there. Higher tiers are considered first, then lower tiers, and the process continues until no additional available stack can be afforded or placed. Recruitment uses the normal creature prices and real dwelling population through the standard castle recruitment path, so population is never duplicated. No treasury percentage is reserved. The old version-9 fractional creature-recruitment bank is cleared and no longer affects purchases. Offline gold income remains boosted to 175% of normal saved gold income.

The state file is upgraded to version 9 to store fractional offline creature-recruitment progress alongside the persistent creature roster, overflow reserve, Supply Rush meter, map/player-state snapshot, treasure fragments, contracts, Renown, streaks, and lifetime offline seconds. Version-1 through version-8 files migrate automatically; new creature-recruitment carry begins at zero. Exact resources, uncapped elapsed time, and resource fractional carry remain compatible.
