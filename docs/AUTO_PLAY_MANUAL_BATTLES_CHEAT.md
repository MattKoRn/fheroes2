...
- Adventure-map interactions also show a **5-second auto-choice popup** before execution. This covers pickups, chests, artifacts, mines, shrines, stat/luck/morale sites, dwellings, events, teleports, monster/hero encounters, boats, barriers, and other actionable adventure objects. Auto-play selects **Yes** after 5 seconds; choosing **No** manually stops F8 auto-play before the action occurs.
- Choice-heavy adventure interactions show additional detail: map events display their authored event text, treasure chests display the reward selected by the AI, and recruitable adventure dwellings use the normal creature-recruit window.
