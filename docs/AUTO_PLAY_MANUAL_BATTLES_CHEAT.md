...
Meaningful absences now also earn a **Homecoming Reward**. Starting at 30 minutes away, the game grants a deterministic bonus shipment to one resource that actually produced offline income. Longer absences improve the chest tier and bonus: Scout's Satchel (5%), Caravan Crate (10%), Royal Chest (15%), King's Vault (20%), and Legendary Hoard (25%). A themed offline event explains the extra shipment. The event is derived from the completed offline session, so reopening or reloading cannot reroll it. Bonus rewards are added to the same exact persistent wallet and respect the existing resource cap.

The base offline-income calculation, uncapped elapsed time, fractional carry, and `offline_progress.dat` format remain compatible with existing files.
